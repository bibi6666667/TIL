...
쌍방이 관계를 맺도록 할 수도 있다 - User에서 Question으로 접근하거나, Question에서 User로 접근할 수 있다.
...
- `@Lob` : contents와 같이 많은 글자를 넣어야 하는 항목에 다는 어노테이션. 255자 이상의 많은 내용을 넣을 수 있음 (Question에도 추가하기)
- 생성자
...
form 태그에서 `name="이름"`으로 지정된 사항들만 컨트롤러의 매개변수에 해당 이름으로 넘어간다.
